...
public class FileManager {
	...
	public void getFileNParseNAppendXmlBuffer(StringBuffer resultBuffer, String dirFlag, String sourceDir, String sourceDirPrefix, String sourceFilePrefix, String partnerCode, boolean isExcludeNgDataToXml) throws Exception {
		try {
			...
			parsingContentNMakeXmlStringNAppend(resultBuffer, fileContent, dirFlag, partnerCode, isExcludeNgDataToXml);
	...
	private void parsingContentNMakeXmlStringNAppend(StringBuffer resultBuffer, String content, String dirFlag, String partnerCode, boolean isExcludeNgDataToXml) throws Exception {
		...
		for (int i = 0; i < rows.length; i++) {
			...
			if (datas == null)
				continue;
			if (datas[6].equalsIgnoreCase("49") || !isExcludeNgDataToXml) {
				...
				resultBuffer.append(getMasterEle(dirFlag, datas, startTestTimeStr+"~"+endTestTimeStr));
				resultBuffer.append(getDetailEle(dirFlag, datas, isExcludeNgDataToXml));
				totalAppendCount+=1;
			} else {
				continue;
			}
		}
		logger.info(totalAppendCount + " Rows Append!(" + dirFlag + ")");
	}
	
	private String getMasterEle(String dirFlag, String[] datas, String testFrom) throws Exception {
		...
		masterBuff.append(FileUtil.TAB).append(FileUtil.TAB).append("<LOTNO>").append(datas[1]).append(dirFlag).append(datas[7]).append("</LOTNO>").append(FileUtil.RN);
		...
		masterBuff.append(FileUtil.TAB).append(FileUtil.TAB).append("<MODEL>AMB</MODEL>").append(FileUtil.RN);
	...
	private String getDetailEle(String dirFlag, String[] datas, boolean isExcludeNgDataToXml) throws Exception {
		...
		detailBuff.append(FileUtil.TAB).append(FileUtil.TAB).append("<LOTNO>").append(datas[1]).append(dirFlag).append(datas[7]).append("</LOTNO>").append(FileUtil.RN);
		...
		detailBuff.append(FileUtil.TAB).append(FileUtil.TAB).append("<TESTITEM>온도</TESTITEM>").append(FileUtil.RN);
		String testCon = datas[3];
		if (datas[3] != null && datas[3].length() == 3)
			testCon = datas[3].substring(1,3);
		
		String masterVolt = datas[9];
		String maxVolt = datas[10];
		String minVolt = datas[11];

		DecimalFormat format = new DecimalFormat();
        format.applyLocalizedPattern("0.#");
        
		String maxVoltResult = format.format(((Double.parseDouble(maxVolt) - Double.parseDouble(masterVolt))));//(max - master)
		String minVoltResult = format.format(((Double.parseDouble(masterVolt) - Double.parseDouble(minVolt))));//(master - min)
		
		String masterVolteResult = "(" + masterVolt + " +" + maxVoltResult  + ", -" + minVoltResult + "V)";
		
		detailBuff.append(FileUtil.TAB).append(FileUtil.TAB).append("<TESTCON>").append(testCon).append("±10˚C").append(masterVolteResult).append("</TESTCON>").append(FileUtil.RN);
		String testValue = datas[4];
		if (datas[4] != null && datas[4].length() == 3)
			testValue = datas[4].substring(1,3);
		
		String volte = "(" + datas[12] + " V)";
		
		detailBuff.append(FileUtil.TAB).append(FileUtil.TAB).append("<TESTVALUE>").append(testValue).append("˚C").append(volte).append("</TESTVALUE>").append(FileUtil.RN);
	...
	/*
	public int getFileNParseNExportXmlNSendFtp_old(String sourceDir, String sourceDirPrefix, String sourceFilePrefix, String exportDir, String partnerCode, boolean isExcludeNgDataToXml) throws Exception {
		try {
			...
			for (int i = 0; i < rows.length; i++) {
				...
				if (layerFlag.equalsIgnoreCase(dataLayerFlag)) {
					...
					parsingContentNMakeXmlFile_old(fileList, partnerCode, exportDir, layerBuff.toString(), isExcludeNgDataToXml);
	...
	private boolean parsingContentNMakeXmlFile_old(List<File> fileList, String partnerCode, String exportDir, String content, boolean isExcludeNgDataToXml) throws Exception {
